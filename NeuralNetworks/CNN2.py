...
class CNN2:
	...
	def create_feature_vector(self, board, player_id):
		n = len(board)
		feature_vector = [[[0 for _ in range(10)] for _ in range(n)] for _ in range(n)]
		...
		for r in range(n):
			for c in range(n):
				...
				if (r, c) in [(0, 0), (n-1, n-1)]:
					feature_vector[r][c][3] = 2
				elif (r, c) in [(0, n-1), (n-1, 0)]:
					feature_vector[r][c][4] = 3
				else:
					feature_vector[r][c][5] = 6
				...
				feature_vector[r][c][8] = 1 if player_id == 1 else 0
				feature_vector[r][c][9] = 1 if player_id == 2 else 0
